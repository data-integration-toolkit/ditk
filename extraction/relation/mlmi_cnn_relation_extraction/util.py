...
def shuffle_split_source(input_data, train_size = 10000,shuffle=False):
    """Shuffle and split data into train and test subset"""
    data = []
...
def _load_bin_vec(fname, vocab):
    ...
    #word2vec = KeyedVectors.load_word2vec_format(fname, binary=True)
    #vec_dim = word2vec.vector_size
    #print(word2vec)
    #print(type(word2vec))
    #print(vec_dim, "dimensions")
...
def prepare_ids(data_dir, vocab_path):
    for context in ['left', 'middle', 'right', 'txt']:
        data_path = data_dir +  '/source.' + context
        target_path = data_dir+ '/ids.'+ context
        print(data_path)
        if context == 'left':
            bos, eos = True, False
        elif context == 'middle':
