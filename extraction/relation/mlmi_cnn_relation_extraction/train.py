...
def train(train_data, test_data, sent_len = None, vocab_size = None, num_classes = None, multi_label = False, use_pretrain = False):
    # train_dir
    ...
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    FLAGS._parse_flags()
    config = dict(list(FLAGS.__flags.items()))
        ...
        with tf.Session().as_default() as sess:
            ...
            def dev_step(mtest, sess):
                ...
                for batch in test_batches:
                    x_batch, y_batch, _ = list(zip(*batch))
            ...
            for batch in train_batches:
                ...
                x_batch, y_batch, a_batch = list(zip(*batch))
