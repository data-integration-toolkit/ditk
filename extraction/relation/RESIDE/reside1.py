...
class RESIDE(RelationExtraction):
	...
	def load_data(self):
		...
		data = pickle.load(open('./data/riedel_processed.pkl', 'rb'))
	...
	def predict(self, sess, data, wLabels=True, shuffle=False, label='Evaluating on Test'):
		...
		for step, batch in enumerate(self.getBatches(data, shuffle)):
			...
			if step % 100 == 0:
				...
				ind_pred = np.unravel_index(np.argmax(y_pred),y_pred.shape)
				ind_actual = np.unravel_index(np.argmax(y),y.shape)

				x_pred = y_pred[ind_pred]
				x_actual = y[ind_actual]
