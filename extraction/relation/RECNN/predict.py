...
def predict(dataobject,**kwargs):
    with tf.device('/cpu:0'):
        output_file=kwargs.get('output_file','predictions.txt')
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            data_common=conversionutil2.SemEval_to_common(dataobject)
            ...
            for i in range(len(preds)):
                print("Predicted Relation is",utils.label2class[preds[i]])
                print(data_common[i],"\t",dataobject[i][2],"\t",dataobject[i][5],"\t",utils.label2class[preds[i]],"\t",utils.label2class[truths[i]])
                prediction_file.writelines("\t".join([data_common[i],dataobject[i][2],dataobject[i][5],utils.label2class[preds[i]],utils.label2class[truths[i]]+'\n']))
