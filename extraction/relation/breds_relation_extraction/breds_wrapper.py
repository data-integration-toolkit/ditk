...
class BREDS(object):
    ...
    def write_relationships_to_disk(self,out_file=None):
        print("\nWriting extracted relationships to disk")
        if not out_file:
            out_file = 'relationships.txt'
...
def main():
    if len(sys.argv) != 7:
        ...
        configuration = {'max_tokens_away':122, 'min_tokens_away':1,'context_window_size':2,'tag_type':'simple','wUpdt':0.5,'wNeg':2,'wUnk':0.0,'min_pattern_support':2,'alpha':0.1,'beta':0.8,'gamma':0.1,'word2vec_path':'afp_apw_xin_embeddings.bin','number_iterations':4}
