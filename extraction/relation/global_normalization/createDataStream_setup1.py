...
class DataStream_Setup1:
  ...
  def load_dataset(self, fileName, dataset_name):
    ...
    self.contextsize = int(config["contextsize"])
    ...
    read_dataset_output_file = data_filename
    ...
    if(dataset_name == 'CoNLL04'):
      ...
      self.train_id2sent, self.train_id2pos, self.train_id2ner, self.train_id2nerBILOU, self.train_id2arg2rel = data_object.preprocess_data(dataset_name, fileName[0], self.configfile)
      self.test_id2sent, self.test_id2pos, self.test_id2ner, self.test_id2nerBILOU, self.test_id2arg2rel = data_object.preprocess_data(dataset_name, fileName[1], self.configfile)


    x1Train, x2Train, x3Train, x4Train, e1Train, e2Train, yTrain, yE1Train, yE2Train, idTrain, e1IdTrain, e2IdTrain = self.processSamples(
        self.train_id2sent, self.train_id2ner, self.train_id2arg2rel, wordindices)
    ...
    f.close()
    output_list = []
    for id in self.test_id2sent:
      sentence = self.test_id2sent[id]
      actual_relation = ''
      for pair in self.test_id2arg2rel[id]:
        actual_relation = self.test_id2arg2rel[id][pair]
      predicated_relation = ''
      output_list.append(sentence+", "+actual_relation)

    return read_dataset_output_file, self.test_id2sent, self.test_id2arg2rel
  ...
  def processSamples(self,id2sent, id2ner, id2arg2rel, wordindices):
    x1List = []
    x2List = []
    x3List = []
    x4List = []
