...
class RNN_Relation(object):
    def __init__(self, num_classes, word_dict_size, d1_dict_size, d2_dict_size, type_dict_size, sentMax, wv,
                 w_emb_size=50, d1_emb_size=5, d2_emb_size=5, type_emb_size=5, num_filters=100, l2_reg_lambda=0.02,test=False):
        if not test:
        ...
        X = tf.concat([tf.cast(emb0,tf.float32), emb3, emb4], 2)  # (N,M,65)
        # print('X', X.get_shape())
    ...
    #     saver = tf.train.import_meta_graph('saved_models/model_1.ckpt.meta')
    #     saver.restore(self.sess, tf.train.latest_checkpoint('saved_models/'))
    #     graph=tf.get_default_graph()
    #     self.global_step=graph.get_operation_by_name('global_step')
    #     self.loss=graph.get_operation_by_name('loss')
    #     self.accuracy=graph.get_operation_by_name('accuracy')
    #     self.predictions=graph.get_operation_by_name('predictions')
