...
class GCNNer:
    ...
    def get_entity_tuples_from_sentence(self, sentence):
        ...
        try:
            (words, embeddings, idx, span) = \
                utils.aux.get_words_embeddings_from_sentence(sentence)
            entities = utils.tuples.get_entities_from_tuple(words,
                    embeddings, self._ner, self._trans_prob)
            return utils.tuples.clean_tuples(words, entities, idx, span)
        except Exception as e:
            self._logger.warning('Cannot process the following sentence: '
                                  + sentence)
            return []
    ...
    def get_entity_tuples_from_text(self, text):
        ...
        all_idx = []
        all_span = []
        for (words, embeddings, idx, span) in sentences:
            try:
                entities = utils.tuples.get_entities_from_tuple(words,
                        embeddings, self._ner, self._trans_prob)
                ...
                all_idx.extend(idx)
                all_span.extend(span)
            except Exception as e:
                self._logger.warning('Cannot process the following sentence: '
                         + ' '.join(words))
        return utils.tuples.clean_tuples(all_words, all_entities,
                all_idx, all_span)

    def test(self, predictions = "", groundTruths = "", dataset = ""):
        '''
        ...
        '''
        if predictions != "" and groundTruths != "":
            tp = 0
            fp = 0
            fn = 0
            tn = 0
            for each in predictions:
                word = groundTruths[predictions.index(each)][0]
                true_type = groundTruths[predictions.index(each)][1]
                predict_type = each[1]
                if true_type == 'O' and predict_type == 'O':
                    tn+=1
                elif true_type == 'O' and predict_type != 'O':
                    fp+=1
                elif true_type != 'O' and predict_type != 'O' and true_type == predict_type:
                    tp+=1
                else:
                    fn+=1
            precision = tp / (tp + fp)
            ...
            sentences = utils.aux.get_all_sentences_train(dataset)
            (data, _) = utils.aux.get_data_from_sentences(sentences)
            (precision, recall, f1) = \
                utils.testing.get_gcn_results(self._ner, data,
                    self._trans_prob)
    ...
    def train_and_save(
        ...
        return utils.training.train_and_save(dataset, saving_dir,
                epochs, bucket_size)
