    ...
    for i,item in enumerate(dataflat[:-1]):
        if item[1] != 'O':
            ...
            if dataflat[i+1][1][2:] != item[1][2:] or dataflat[i+1][1][0] == 'B':
                entitytype = item[1][2:]
                # Kunal Additions
                final_result.append((''.join(ent), entitytype))
                entities[entitytype].append((start_pos, end_pos,  entitytype, tuple(ent) ) )
    # with open("result.txt", "w", encoding='utf-8',errors='ignore') as opfile:
    #     for obj in final_result:
    #         opfile.write(obj[0] + "\t" + obj[1] +'\n')
...
def fix_i_b(sents, fix_or_check):
    ...
    if fix_or_check == 'check':
        ...
        raise ValueError("Use 'fix' or 'check'.")
