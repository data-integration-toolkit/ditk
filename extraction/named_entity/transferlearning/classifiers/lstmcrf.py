...
def predict(model, test, tag2idx, word2idx, tags, max_len, batch_size=32):
    ...
    ps = [model.predict( [ np.array([X_te[i]]), np.array([X_ca[i]]) ], batch_size=batch_size) for i in range(len(test))]
    ps = [np.argmax(x, axis=-1) for x in ps]

    A = [ [(test[jj][ii][0], tags[pred] ) for ii, pred in enumerate(ps[jj][0]) if X_te[jj][ii]!=0 ] for jj in range(len(test))]
...
def make_biLSTM_casing_CRF2(n_tags, n_words, max_len, pretrained_embeddings = None,
    ...
    if pretrained_embeddings is not None:
        ...
        model.get_layer(name=we_name).set_weights([pretrained_embeddings])
