...
def save_model(config, pre_trained=False):
    raw_data = joblib.load(config["data_path"])
    ...
    with tf.Session(config=gpu_config, graph=g) as sess:
        ...
        if pre_trained:
            ...
            print("There is no pre-trained model ... so run initial model ... ")
        saver.save(sess, config["save_path"])

    del g
    sess.close()

def load_model(config, pre_trained=False):
    raw_data = joblib.load(config["data_path"])
...
def add_predict(config, data, pre_trained=False):
...
def add_get_score(pred_label, ground_truth_label, config, dataset):
    ...
    portion = "test"
    os.system("%s < %s > %s" % (config["eval_script"],
                                config["output_path"] + "." + portion + ".output",
                                config["output_path"] + "." + portion + ".scores"))

    eval_lines = [l.rstrip() for l in open(config["output_path"] + "." + portion + ".scores", 'r')]
