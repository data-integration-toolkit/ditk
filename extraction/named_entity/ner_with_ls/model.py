...
def evaluate(config, sess, model, dataset, batcher, id_to_word, id_to_tag):
    ...
    with open(config["output_path"] + "_" + dataset + "_output", 'w') as f:
        f.write(st + "\n")

    os.system("%s < %s > %s" % (config["eval_script"],
                                config["output_path"] + "_" + dataset + "_output",
                                config["output_path"] + "_" + dataset + "_scores"))

    # CoNLL evaluation results
    eval_lines = [l.rstrip() for l in open(config["output_path"] + "_" + dataset + "_scores", 'r')]

    val = float(eval_lines[1].strip().split()[-1])

    return avg_cost, val

def pred_evaluate(config, sess, model, dataset, batcher, id_to_word, id_to_tag):
...
def initi_default(raw_data):
    ...
    return config, id_to_word, id_to_tag, emb_matrix, ls_matrix

def add_train(data, config):
...
def add_predict(config, data):
    print("add_predict:", config["features"], config["lstm_word"])
    raw_data = joblib.load(config["data_path"])
    ...
    batcher = {}
    batcher["test"] = Batcher(config, data, id_to_emb_map, word_to_char_mapping, word_to_cap_mapping, False)
    ...
    with tf.Session(config=gpu_config, graph=g) as sess:
        ...
        for key, value in batcher.items():
            tags_pred, tags_gold = pred_evaluate(config, sess, model, key, value, id_to_word, id_to_tag)

    del g
    sess.close()
    return tags_pred, tags_gold

def add_get_score(pred_label, ground_truth_label, config, dataset):

    portion = "test"
    os.system("%s < %s > %s" % (config["eval_script"],
                                config["output_path"] + "_" + portion + "_output",
                                config["output_path"] + "_" + portion + "_scores"))

    eval_lines = [l.rstrip() for l in open(config["output_path"] + "_" + portion + "_scores", 'r')]
    

    precision = eval_lines[1].strip().split()[3]
    recall = eval_lines[1].strip().split()[5]
