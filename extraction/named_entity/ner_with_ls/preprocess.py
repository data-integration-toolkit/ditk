...
def dump_ner_data(config, dataset):
    ...
    for p in config["portion"]:
        data[p] = None
        if dataset== "test_data" and p != "test":
            continue
...
def create_onto_raw(raw_path, portion):
    ...
    filename = os.path.join(raw_path, "ontonotes."+portion+".txt")
...
def read_data(dataset):
    ...
    if dataset == "ontonotes":
        for p in config["portion"]:
            create_onto_raw(config["raw_path"], p)

    elif dataset == "conll":
        for p in config["portion"]:
            create_conll_raw(config["raw_path"], p)

    else:
        print("Unknown dataset")
        sys.exit(1)

    return dump_ner_data(config, dataset)
