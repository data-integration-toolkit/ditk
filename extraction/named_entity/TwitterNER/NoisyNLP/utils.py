...
def parse_results(results):
    ...
    result_obj = dict()
    try:
        result_obj.update((k,v) for k,v in zip(("processed_tokens", "total", "found", "correct"),re.findall(r'[0-9]+', results[0].strip())))
        ...
        for line in results[2:]:
            ...
            try:
                ...
                print ('Excpetion raised for no groups attr in regex')
        # prfs_vals.append(items[:1] + tuple(map(float, items[1:])))
...
def print_results(result_obj):
    ...
    print ("Overall acc couldn't be found")
    print(result_obj["prfs"])
    print ("Overall prfs couldn't be obtained")
    
def get_conll_eval(filename):
    try:
        cmd = "cat \"%s\" | tr '\\t' ' ' | perl -ne '{chomp;s/\\r//g;print $_,\"\\n\";}' | python2 conlleval.py" % filename
    except:
        cmd = "cat \"%s\" | tr '\\t' ' ' | perl -ne '{chomp;s/\\r//g;print $_,\"\\n\";}' | python2 NoisyNLP/conlleval.py" % filename
    #cmd = "cat \"%s\" | tr '\\t' ' ' | perl -ne '{chomp;s/\\r//g;print $_,\"\\n\";}' | python conlleval.py" % filename
    print("Running:\n%s" % cmd)
...
#print (get_conll_eval('/home/khadutz95/TwitterNER/test_exp/test.bieou.tsv'))
