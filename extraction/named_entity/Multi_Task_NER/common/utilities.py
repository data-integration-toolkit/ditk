...
def preprocess_data(fileName):
    ...
    valid_file = fileName['valid']
    test_file = fileName['test']
    predix = ''
    if train_file.rfind('/') >=0:
        predix = train_file[:train_file.rfind('/')] +'/'
    f_label_path_train = predix + 'train.conll.preproc.url.postag'
    f_postag_path_train = predix + 'train.conll.preproc.url'
    f_label_path_dev = predix + 'dev.conll.preproc.url.postag'
    f_postag_path_dev = predix + 'dev.conll.preproc.url'
    f_label_path_test = predix + 'test.conll.preproc.url.postag'
    f_postag_path_test = predix + 'test.conll.preproc.url'

    file = open(train_file,'r')
    f_label = open(f_label_path_train, 'w')
    f_postag = open(f_postag_path_train, 'w')

    for line in file.readlines():
        line = line.split()
        if len(line) >= 4:
            f_label.write(line[0] + '\t' + line[1] + '\n')
            f_postag.write(line[0] + '\t' + line[3] + '\n')

            if line[3] not in entities:
                entities.append(line[3])
            if line[3] not in stored_tags:
                stored_tags.append(line[3])
    file.close()
    f_label.close()
    f_postag.close()

    file = open(valid_file,'r')
    f_label = open(f_label_path_dev, 'w')
    f_postag = open(f_postag_path_dev, 'w')
    for line in file.readlines():
        line = line.split()
        if len(line) >= 4:
            f_label.write(line[0] + '\t' + line[1] + '\n')
    ...
    f_label = open(f_label_path_test, 'w')
    f_postag = open(f_postag_path_test, 'w')
    for line in file.readlines():
        line = line.split()
        if len(line) >= 4:
            f_label.write(line[0] + '\t' + line[1] + '\n')
    ...
    diff = list(set(entities) - set(stored_tags))
    if len(diff) > 0:
        f_postag = open(f_postag_path_train, 'a+')
        f_label = open(f_label_path_train, 'a+')
        for d in diff:
            f_postag.write('extra' + '\t' + d + '\n')
            f_label.write('extra' + '\t' + 'extra' + '\n')
        f_postag.close()
        f_label.close()
    ...
    file.close()
    f_label_train = open(f_label_path_train, 'w')
    f_postag_train = open(f_postag_path_train, 'w')
    f_label_valid = open(f_label_path_dev, 'w')
    f_postag_valid = open(f_postag_path_dev, 'w')
    f_label_test = open(f_label_path_test, 'w')
    f_postag_test = open(f_postag_path_test, 'w')
    print("lines:  "+ str(len(lines)))
    if len(lines) < 1:
        for line in lines:
            line = line.split()
            if len(line) > 4:
                f_label_train.write(line[0] + '\t' + line[1] + '\n')
                f_postag_train.write(line[0] + '\t' + line[3] + '\n')
                f_label_valid.write(line[0] + '\t' + line[1] + '\n')
                f_postag_valid.write(line[0] + '\t' + line[3] + '\n')
                f_label_test.write(line[0] + '\t' + line[1] + '\n')
                f_postag_test.write(line[0] + '\t' + line[3] + '\n')
        ...
        f_label_train.close()
        f_postag_train.close()

        f_label = open(f_label_path_dev, 'w')
        f_postag = open(f_postag_path_dev, 'w')
        for line in lines[int(len(lines)/2):int(3*len(lines)/4)]:
            line = line.split(' ')
            if len(line) >4:
                f_label.write(line[0] + '\t' + line[1] + '\n')
                f_postag.write(line[0] + '\t' + line[3] + '\n')
        ...
        f_label = open(f_label_path_test, 'w')
        f_postag = open(f_postag_path_test, 'w')
        print('test: '+str(int(3*len(lines)/4)))

        for line in lines[int(3*len(lines)/4):]:
            line = line.split(' ')
            if len(line) >4:
                f_label.write(line[0] + '\t' + line[1] + '\n')
                f_postag.write(line[0] + '\t' + line[3] + '\n')
        ...
        f_label.close()
        f_postag.close()
    if len(stored_tags) != len(entities):
        diff = list(set(entities) - set(stored_tags))
        f_postag = open(f_postag_path_train, 'a+')
        f_label = open(f_label_path_train, 'a+')
        for d in diff:
...
def read_and_sync_postags(tweets_train, tweets_test,TRAIN_PREPROC_URL_POSTAG,DEV_PREPROC_URL_POSTAG,TEST_PREPROC_URL_POSTAG):
