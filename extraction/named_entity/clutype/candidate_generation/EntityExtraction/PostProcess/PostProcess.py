from candidate_generation.EntityExtraction.RelationExtraction import RelationConstruction
from candidate_generation.EntityExtraction.Patterns import VerbPhrase
from candidate_generation.EntityExtraction.PPV import PPV
...
class PostProcess:
    ...
    def original(self, sentence, pos_tags):
        ...
        for i in xrange(len(sentence)):
            phrase = sentence[i]
            phrase_pos = pos_tags[i]
            if (phrase_pos[0].startswith("NN") or phrase_pos[0].startswith("JJ")) and any(
                ...
                if not len(final) == 1 or first_part[0][0].isupper():
                    ...
                    1 if not (x.startswith("NN") or x.startswith("JJ")) else 0 for x in phrase_pos) > 3:
                for j in xrange(len(phrase_pos)):
