...
class Clean:
    def __init__(self, path, full_sentence, full_pos, sentences, pos, phrase_segment_path):
        self.Documents = []
        ...
        self.allowed = set([chr(i) for i in xrange(ord('a'), ord('z') + 1)] + \
                           ...
                           [',', '.', '?', '-', '!', ' '] + [str(i) for i in xrange(10)])
        self.punctuation = [';', ':', '&', '?', "/"]
        self.P = Partition(self.punctuation, phrase_segment_path)
    ...
    def clean_and_tag(self):
        with open(self.full_sentence, 'w') as f, \
                open(self.full_pos, 'w') as g, \
                open(self.sentences, 'w') as m, \
                open(self.pos, 'w') as n:
