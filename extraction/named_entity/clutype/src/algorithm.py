...
def clustype_appx(S_L, S_R, S_M, PiC, PiL, PiR, Y0, lambda_O, gamma, mu, T, ITER, K):
    ...
    for i in range(ITER):
        ...
        PL = inverse_matrix(identity(PiL.shape[1]) + lambda_O * PiLL) * (
                S_L.T * C + lambda_O * PiL.T * (Y - PiC * C - PiR * PR))
        PR = inverse_matrix(identity(PiR.shape[1]) + lambda_O * PiRR) * (
                S_R.T * C + lambda_O * PiR.T * (Y - PiC * C - PiL * PL))
...
def clustype_noClus_PiC(S_L, S_R, S_M, PiC, PiL, PiR, Y0, lambda_O, gamma, mu, T, ITER):
    ...
    Y = Y0.copy()
    obj = trace(2 * C.T * C + PL.T * PL + PR.T * PR - 2 * C.T * S_L * PL - 2 * C.T * S_R * PR) + \
          lambda_O * (norm(Y - PiC * C, ord='fro') ** 2 + mu * norm(Y - Y0, ord='fro') ** 2 + gamma * trace(
        Y.T * Y - Y.T * S_M * Y))
...
def clustype_onlycandidate(S_L, S_R, PiC, PiL, PiR, Y0, T, ITER):
    ...
    Theta = PiC * C + PiL * PL + PiR * PR
    obj = trace((
                        2 + u) * C.T * C + PL.T * PL + PR.T * PR - 2 * C.T * S_L * PL - 2 * C.T * S_R * PR - 2 * u * C.T * C0 + u * C0.T * C0)
