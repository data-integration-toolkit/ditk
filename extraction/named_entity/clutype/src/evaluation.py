...
def write_output(annotation_path, Y, doc_mid, mid_mention, tid_type):
    ...
    for doc_id in doc_mid:
        for mid in doc_mid[doc_id]:
            ...
            results.append([None, None, mention_name, tid_type[predict_tid]])
            f.write(mention_name + '\t' + mapping[tid_type[predict_tid].lower()] + '\n')
    f.close()
    return results
                ...
                if mention_name + ':ep' in sentence_j_text:
                    for i in range(len(sentence_list[j])):
                        if mention_name + ':ep' == sentence_list[j][i].lower():
                            doc_text[doc_id][j][i] = '[' + sentence_list[j][i][:-3] + ']:' + tid_type[
                                predict_tid].upper()
...
def evaluate(gt_dict, pred_dict):
    ...
    print 'F1 = ', 2 * (overlap_size / pred_size * overlap_size / gt_size) / (
            overlap_size / pred_size + overlap_size / gt_size)
