...
def run(corpus_path, stat_path, stop_path):
    ### Parameters #############################################################
    ...
    top_dir = os.path.dirname(os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))) + "/"
    ...
    fff.write('CANDIDATE_MINSUP=' + str(CANDIDATE_MINSUP) + ', RP_CONTEXT_WINDOW_SIZE=' + str(
        RP_CONTEXT_WINDOW_SIZE) + ', K_SM=' + str(K_SM) + '\n')

    PiC_path = top_dir + 'temp/temp_gc/PiC.txt'
    cid_path = top_dir + 'temp/temp_gc/candidate_cid.txt'
    mid_path = top_dir + 'temp/temp_gc/mention_mid.txt'  # Y_0 = (mention, mid, initial_score)
    pid_path = top_dir + 'temp/temp_gc/RP_pid.txt'
    PiL_path = top_dir + 'temp/temp_gc/PiL.txt'
    PiR_path = top_dir + 'temp/temp_gc/PiR.txt'
    mention_graph_path = top_dir + 'temp/temp_gc/W_M.txt'  # mention-mention graph
    F_string_path = top_dir + 'temp/temp_gc/F_string.txt'
    string_wid_path = top_dir + 'temp/temp_gc/string_wid.txt'
    F_context_path = top_dir + 'temp/temp_gc/F_context.txt'
    context_wid_path = top_dir + 'temp/temp_gc/context_wid.txt'
                        ...
                        if len(left_RPs) > 0 or len(right_RPs) > 0:
                            ...
                            for triple in left_RPs + right_RPs:
                                ...
                                if end_idx < (len(term_list) - 1):
                                    right_window = term_list[
                                                   (end_idx + 1): min(end_idx + 1 + RP_CONTEXT_WINDOW_SIZE,
                                                                      len(term_list))]
                                else:
                                    right_window = []
    ...
    print '#mention before filter:', len(mention_context), ', #PiL rows:', len(PiL_RPs), ', #PiR rows:', len(
        PiR_RPs), ', #candidates before filter:', len(candidate_degree)
    fff.write('#mention before filter: ' + str(len(mention_context)) + ', #candidates before filter:' + str(
        len(candidate_degree)) + ', #RPs before filter' + str(len(RP_context)) + '\n')
    ...
    print '#mentions after filter:', len(mention_mid), ', PiC #rows:', len(PiC), ', #candidates after filter:', len(
        candidate_cid)
    ...
    fff.write('#mention after filter: ' + str(len(mention_mid)) + ', #candidates after filter:' + str(
        len(candidate_cid)) + ', candidate avg#mentions' + str(
        (mid + 0.0) / cid) + ', candidate, max# mentions: ' + a + ', ' + str(b) + '\n')
    del PiC
    ...
    F_string, F_context, string_wid, context_wid = feature_generation(RP_pid, RP_context, RP_context_DF, len(doc_set),
                                                                      stoplist)
