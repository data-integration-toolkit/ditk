...
def multiNMF_onestep(PL, Ul, Vl, betaWL, obj_WL, PR, Ur, Vr, betaWR, obj_WR, \
                     F_string, Us, Vs, betaFs, obj_Fs, F_context, Uc, Vc, betaFc, obj_Fc, alpha, K, INNER_ITER, tol,
                     VERBOSE):
    ## update Ustar
...
def clustype_inexact(S_L, S_R, S_M, PiC, PiL, PiR, F_context, F_string, Y0, lambda_O, gamma, mu, lambda_L, alpha, T, K,
                     ITER, INNER_ITER, tol, Y_cand, C_cand, PL_cand, PR_cand, VERBOSE):
    PiLL = PiL.T * PiL
    PiRR = PiR.T * PiR
    ...
    obj = trace(2 * C.T * C + PL.T * PL + PR.T * PR - 2 * C.T * S_L * PL - 2 * C.T * S_R * PR) + \
          lambda_O * (norm(Y - Theta, ord='fro') ** 2 + mu * norm(Y - Y0, ord='fro') ** 2 + gamma * trace(
        Y.T * Y - Y.T * S_M * Y))
    ...
    for i in range(ITER):
        ...
        PL = inverse_matrix((1 + lambda_L * betaWL) * identity(PiL.shape[1]) + lambda_O * PiLL) * (
                    S_L.T * C + lambda_O * PiL.T * (Y - PiC * C - PiR * PR) + lambda_L * betaWL * Ul * Vl.T)
        PR = inverse_matrix((1 + lambda_L * betaWR) * identity(PiR.shape[1]) + lambda_O * PiRR) * (
                    S_R.T * C + lambda_O * PiR.T * (Y - PiC * C - PiL * PL) + lambda_L * betaWR * Ur * Vr.T)
        ...
        (Ul, Vl, betaWL, obj_WL, Ur, Vr, betaWR, obj_WR, Us, Vs, betaFs, obj_Fs, Uc, Vc, betaFc, obj_Fc, obj_multiNMF) = \
            multiNMF_onestep(PL, Ul, Vl, betaWL, obj_WL, PR, Ur, Vr, betaWR, obj_WR, \
                             F_string, Us, Vs, betaFs, obj_Fs, F_context, Uc, Vc, betaFc, obj_Fc, alpha, K, INNER_ITER,
                             tol, VERBOSE)
        ...
        obj = trace(2 * C.T * C + PL.T * PL + PR.T * PR - 2 * C.T * S_L * PL - 2 * C.T * S_R * PR) + \
              lambda_O * (norm(Y - Theta, ord='fro') ** 2 + mu * norm(Y - Y0, ord='fro') ** 2 + gamma * trace(
            Y.T * Y - Y.T * S_M * Y)) + lambda_L * obj_multiNMF
        rel = (obj_old - obj) / obj_old
        if (i + 1) % 1 == 0:
...
def multiNMF(PL, Ul, Vl, betaWL, obj_WL, PR, Ur, Vr, betaWR, obj_WR, \
             F_string, Us, Vs, betaFs, obj_Fs, F_context, Uc, Vc, betaFc, obj_Fc, Ustar, alpha, K, OUTER_ITER,
             INNER_ITER, tol, VERBOSE):
    obj_multiNMF = betaWL * obj_WL + betaWR * obj_WR + betaFs * obj_Fs + betaFc * obj_Fc
    for iter in range(OUTER_ITER):
        ...
        if abs(rel) < tol:
            ...
            return (
            Ul, Vl, betaWL, obj_WL, Ur, Vr, betaWR, obj_WR, Us, Vs, betaFs, obj_Fs, Uc, Vc, betaFc, obj_Fc, Ustar,
            obj_multiNMF)
...
def clustype_exact(S_L, S_R, S_M, PiC, PiL, PiR, F_context, F_string, Y0, lambda_O, gamma, mu, lambda_L, alpha, T, K,
                   ITER, OUTER_ITER, INNER_ITER, tol, Y_cand, C_cand, PL_cand, PR_cand, VERBOSE):
    PiLL = PiL.T * PiL
    PiRR = PiR.T * PiR
    ...
    for i in range(ITER):
        ...
        Y, C, PL, PR = clustype_noClus_inner(S_L, S_R, S_M, PiC, PiL, PiR, Y0, lambda_O, gamma, mu, T, INNER_ITER, tol,
                                             C, PL, PR, Y)
...
def clustype_uniform(S_L, S_R, S_M, PiC, PiL, PiR, F_context, F_string, Y0, lambda_O, gamma, mu, lambda_L, alpha, T, K,
                     ITER, INNER_ITER, tol, Y_cand, C_cand, PL_cand, PR_cand, VERBOSE):
    PiLL = PiL.T * PiL
    PiRR = PiR.T * PiR
    ...
    for i in range(ITER):
        ...
        (Ul, Vl, betaWL, obj_WL, Ur, Vr, betaWR, obj_WR, Us, Vs, betaFs, obj_Fs, Uc, Vc, betaFc, obj_Fc, obj_multiNMF) = \
            multiNMF_onestep_fixBeta(PL, Ul, Vl, betaWL, obj_WL, PR, Ur, Vr, betaWR, obj_WR, \
                                     F_string, Us, Vs, betaFs, obj_Fs, F_context, Uc, Vc, betaFc, obj_Fc, alpha, K,
                                     INNER_ITER, tol, VERBOSE)
...
def clustype_noFsFc(S_L, S_R, S_M, PiC, PiL, PiR, F_context, F_string, Y0, lambda_O, gamma, mu, lambda_L, alpha, T, K,
                    ITER, INNER_ITER, tol, Y_cand, C_cand, PL_cand, PR_cand, VERBOSE):
    PiLL = PiL.T * PiL
    PiRR = PiR.T * PiR
