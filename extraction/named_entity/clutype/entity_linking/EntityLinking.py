...
TEMP_PATH = os.path.join(top_dir, 'temp/temp/')
...
class myLinker(threading.Thread):
    ...
    def __init__(self, docList, offset, confidence, default_types):
    ...
    def run(self):
        ...
        g = open(TEMP_PATH + 'entity_linking_temp' + str(self.offset) + '.txt', 'w')
        index = 0
        while 1:
            ...
            if did in self.docList:
                try:
                    ...
                    data = dict({"confidence": self.confidence})
                    data["support"] = "20"
                    data["text"] = doc
                    ...
                    for entity in validEntities:
                        ...
                        if (entity['@URI'] != None):
                            new_types = []
                            entity_types = entity.get('@types', None)
                            if entity_types is not None and len(entity_types) > 0:
                                entity_types = entity_types.lower()
                                for default_type in self.default_types:
                                    if entity_types.find(default_type.split('/')[1]) != -1:
                                        new_types.append(default_type)
                            if len(new_types) == 0:
                                final_str = str(index + self.offset) + '\t' + entity[
                                    '@surfaceForm'] + '\t' + "" + '\t' + entity['@URI'] + '\t' + entity[
                                                '@similarityScore'] + '\t' + entity['@percentageOfSecondRank'] + '\n'
                            else:
                                final_str = str(index + self.offset) + '\t' + entity['@surfaceForm'] + '\t' + ";".join(
                                    new_types) + '\t' + entity['@URI'] + '\t' + entity[
                                                '@similarityScore'] + '\t' + entity['@percentageOfSecondRank'] + '\n'
                            g.write(final_str)
                    index += threadNum
                except:
...
def link(docFile, confidence, typefile):
    # typeDict['m.01c5'] = None
    # read documents
    default_types = []
    with open(typefile) as fout:
        for line in fout:
            myType = line.split('\t')[0]
            if myType != 'NIL':
                default_types.append(myType)
    ...
    for i in range(0, threadNum):
        mythread = myLinker(docList, i, confidence, default_types)
        mythread.start()
    ...
    for i in range(0, threadNum):
        f = open(TEMP_PATH + 'entity_linking_temp' + str(i) + '.txt')
        for line in f:
            tab = line.find('\t')
            did = int(line[:tab])
            allLines[line] = did
    ...
    g = open(TEMP_PATH + 'entity_linking_temp.txt', 'w')
    for tup in sorted_x:
        g.write(tup[0])
    g.close()
...
#     f = open(TEMP_PATH + 'entity_linking_temp.txt')
#     idToType = {}
#     # Get all the types
#     for line in f:
#         segments = line.strip('\r\n').split('\t')
...
#             continue
#
#     g = open(TEMP_PATH + 'entity_linking_temp_2.txt', 'w')
#     f.close()
#     f = open(TEMP_PATH + 'entity_linking_temp.txt')
#     # print typeDict
...
def filterTypes(typeFile, outFile, freebasekey):
    ...
    # f = open(TEMP_PATH + 'entity_linking_temp_2.txt')
    f = open(TEMP_PATH + 'entity_linking_temp.txt')
    g = open(outFile, 'w')
    typeF = open(typeFile)
    for line in typeF:
    ...
    for line in f:
        ...
        g.write(segments[0] + '\t' + segments[1] + '\t' + segments[2] + '\t' + segments[2] + '\t' + segments[3]
