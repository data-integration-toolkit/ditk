...
def generate_corpus_char_from_data(data, if_shrink_c_feature=False, c_thresholds=1, if_shrink_w_feature=False, w_thresholds=1):
    """
    generate label, feature, word dictionary, char dictionary and label dictionary
    ...
    """
    features, labels, feature_map, label_map = generate_corpus_from_data(data, if_shrink_feature=if_shrink_w_feature, thresholds=w_thresholds)
    char_count = dict()
    for feature in features:
        for word in feature:
            for tup in word:
...
def generate_corpus_from_data(data, if_shrink_feature=False, thresholds=1):
    """
    generate label, feature, word dictionary and label dictionary
    ...
    for line in data:
        if len(line) != 0 and len(line[0].strip()) != 0:
            ...
            if line[3] not in label_map:
                label_map[line[3]] = len(label_map)
        elif len(tmp_fl) > 0:
            features.append(tmp_fl)
            labels.append(tmp_ll)
            tmp_fl = list()
...
def read_features_from_data(data, multi_docs = True):
    """
    ...
    for line in data:
        if len(line) != 0 and len(line[0].strip()) != 0:
            ...
            if len(tmp_fl) > 0:
                features.append(tmp_fl)
                tmp_fl = list()
            if len(line) == 0 and len(features) > 0:
