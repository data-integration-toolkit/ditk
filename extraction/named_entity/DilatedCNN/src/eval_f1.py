...
def segment_eval(batches, predictions, label_map, type_int_int_map, labels_id_str_map, vocab_id_str_map, outside_idx, pad_width, start_end, extra_text="", verbose=False):
    ...
    with open('eval.txt', 'w') as eval_file:
        print("\t%10s\tPrec\tRecall" % ("F1"), file=eval_file)
        print("%10s\t%2.2f\t%2.2f\t%2.2f" % ("Micro (Seg)", f1_micro * 100, precision_micro * 100, recall_micro * 100), file=eval_file)
        print("%10s\t%2.2f\t%2.2f\t%2.2f" % ("Macro (Seg)", f1_macro * 100, precision_macro * 100, recall_macro * 100), file=eval_file)
        print("-------")
    for t in label_map:
        idx = label_map[t]
