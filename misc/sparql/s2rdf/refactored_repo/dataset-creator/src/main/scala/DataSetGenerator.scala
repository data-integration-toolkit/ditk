...
object DataSetGenerator {
  ...
  private val _sc = Settings.sparkSession.sparkContext
  private val _sqlContext = Settings.sparkSession.sqlContext
  ...
  def generateDataSet(datasetType: String) = {
    ...
    if (datasetType == "VP") {
      // New for csci548 - Need to new create statistics directory and files
      Helper.removeDirInHDFS(Settings.statisticsDir)
      Helper.createDirInHDFS(Settings.statisticsDir)
  ...
  private def createVP() = {
    ...
    // New for CSCI-548 - Create statistics directory
    Helper.removeDirInHDFS(Settings.statisticsDir)
    Helper.createDirInHDFS(Settings.statisticsDir)
