...
object QueryExecutor {
  ...
  private val _sc =  Settings.sparkSession.sparkContext  //Settings.sparkContext
  private val _sqlContext =  Settings.sparkSession.sqlContext //Settings.sqlContext
    ...
    for (tableStat <- qr.tables.values){
      ...
      if (_cachedTables contains fileName){
        ...
        if (actualPostfix.contains("SO") && Settings.partNumberExtVP != 200)
          ...
          table = _sqlContext.read.parquet(fileName) //read.parquet(fileName)
                  .repartition(Settings.partNumberVP);
        else
          table = _sqlContext.read.parquet(fileName)
        table.createOrReplaceTempView(tableStat.tName)//registerTempTable(tableStat.tName);
