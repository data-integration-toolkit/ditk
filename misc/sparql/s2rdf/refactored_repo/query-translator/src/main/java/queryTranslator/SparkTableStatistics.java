...
public class SparkTableStatistics {
	...
	public static HashMap<String, TStat> ssStats = new HashMap<>();
	...
	private static long readExtVPStatistic(String relType){
		...
		try(BufferedReader reader = new BufferedReader(new FileReader(getExtVPStatFileDest(relType)))) {
		  String line;
		  while ((line = reader.readLine()) != null){
			  String[] temp = line.split(",");
			  TStat newStat = new TStat(temp[1], temp[2], temp[3]);
			  ...
			  if (relType.equals("SO")) soStats.put(temp[0].replace(":","__")
					                                       .replace("<<", "<")
					                                       .replace(">>", ">")
					                                        ,newStat);
			  else if (relType.equals("OS")) osStats.put(temp[0].replace(":","__")
					                                            .replace(":","__")
					                                            .replace("<<", "<")
					                                            .replace(">>", ">")
					                                             ,newStat);
			  else if (relType.equals("SS")) ssStats.put(temp[0].replace(":","__")
					                                            .replace("<<", "<")
					                                            ,newStat);
		  }
		} catch (Exception e) { // There was no tables for this relation type
	...
	public static String generateTablesUsageInstructions(String query){
		...
		for (String placeHolderName:tableCandidatesMap.keySet()){
			...
			String newPlaceholder = placeHolderName.replace(":","__").replace("<", "_L_").replace(">", "_B_");
			query = query.replace(placeHolderName, newPlaceholder);
			res+=newPlaceholder+"\t" + candList.bestCandidateID + "\t" + bestCandidate[0].replace(":","__")
					+ "\t" + bestCandidate[1].replace(":","__").replace("<", "_L_").replace(">", "_B_")
					+ "/" + bestCandidate[2].replace(":","__").replace("<", "_L_").replace(">", "_B_")
					+"\n";
