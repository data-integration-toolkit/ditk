...
def epoch_train(model, optimizer, batch_size, sql_data, table_data, pred_entry):
    ...
    while st < len(sql_data):
        ...
        new_loss = loss.data.cpu().numpy()
        #print new_loss.shape
        #print new_loss
        if new_loss.shape == ():
            cum_loss += new_loss*(ed - st)
        else:
            cum_loss += new_loss[0]*(ed-st)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def epoch_exec_acc(model, batch_size, sql_data, table_data, db_path, pred_queries_holder=None):
    ...
    while st < len(sql_data):
        ...
        if pred_queries_holder is not None:
            pred_queries_holder.append(pred_queries)
...
def epoch_acc(model, batch_size, sql_data, table_data, pred_entry, pred_queries_holder=None):
    model.eval()
    perm = list(range(len(sql_data)))
    st = 0
    one_acc_num = 0.0
