...
featurePath = None
feature_stats = None
relationId_path = None

def setup(rel, dirname) :
	global dataPath_, featurePath, feature_stats, relationId_path, relation
	relation = rel.replace(":", "_")
	dataPath_ = dirname + "/tasks/" + relation
	featurePath = dataPath_ + '/path_to_use.txt'
	feature_stats = dataPath_ + '/path_stats.txt'
	relationId_path = dirname + '/relation2id.txt'
...
def train(kb, kb_inv, named_paths):
	...
	if len(training_features) > 0 and len(train_labels) > 0 :
...
def update_data (dir_name, relation) :
	dataPath_ = '../' + dir_name + '/tasks/'  + relation
	featurePath = dataPath_ + '/path_to_use.txt'
	feature_stats = dataPath_ + '/path_stats.txt'
	relationId_path = '../' + dir_name + '/relation2id.txt'


if __name__ == '__main__':
	setup(sys.argv[1], "../NELL-995")
	evaluate_logic()
