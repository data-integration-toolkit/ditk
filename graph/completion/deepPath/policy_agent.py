...
def setup(rel, path, t) :
	global dataPath, relation, task, graphPath, relationPath
	dataPath = path
...
def retrain(optional_relation = None):
	if optional_relation != None :
	...
	with tf.Session() as sess:
		...
		if episodes == 0 :
			print ('Cannot train model not training data given for relation ', relation)
			return
		if episodes > 300:
			episodes = 300
		print(relation)
...
def test(optional_relation = None):
	if optional_relation != None :
        	relation = optional_relation.replace(":", "_")
        	relationPath = dataPath + 'tasks/' + relation.replace(":", "_") + '/' + 'train_pos'
...
if __name__ == "__main__":
	setup(sys.argv[1], dataPath, sys.argv[2])
	if task == 'test':
		test()
	elif task == 'retrain':
		retrain()
