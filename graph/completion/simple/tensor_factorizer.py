...
class TensorFactorizer:
	...
	def setup_reader(self, data=None, reader=None):
		if reader:
			self.reader = reader
		else:
			self.reader = Reader()
		...
		if data:
			self.reader.triples["train"] = data
		else:
	...
	def test(self, triples):
		...
		for i, triple in enumerate(triples):
			...
			for i, _ in enumerate(head_fil_preds):
				h_prob = 1 - sigmoid(head_fil_preds[i]) # REVIEW
				if h_prob > threshold:
					new_triples.add((head_fil_h[i], head_fil_r[i], head_fil_t[i]))

			for j, _ in enumerate(tail_fil_preds):
				t_prob = 1 - sigmoid(tail_fil_preds[j])
				if t_prob > threshold:
					new_triples.add((tail_fil_h[j], tail_fil_r[j], tail_fil_t[j]))
		...
		return r_mrr, r_hit1, r_hit3, r_hit10, f_mrr, f_hit1, f_hit3, f_hit10, new_triples
