    ...
    for file in ["train", "valid", "test"]:
        with open(os.path.join(directory, file + ".txt"), "r") as f:
            for line in f.readlines():
                ...
                head_id = get_add_ent_id(head, ent2id)
                tail_id = get_add_ent_id(tail, ent2id)
                rel_id = get_add_rel_id(rel, rel2id)
...
def get_add_ent_id(entity, ent2id):
    if entity in ent2id:
...
def get_add_rel_id(relation, rel2id):
    if relation in rel2id:
        relation_id = rel2id[relation]
    else:
        relation_id = len(rel2id)
        rel2id[relation] = relation_id
    return relation_id
...
def build_data(name='WN18', path='../../CNNGraph/data'):
    ...
    train_triples, words_indexes, _ = load_triples_from_txt(os.path.join(folder, 'train.txt'), parse_line=parse_line)

    valid_triples, words_indexes, _ = load_triples_from_txt(os.path.join(folder, 'valid.txt'),
                                                            words_indexes=words_indexes, parse_line=parse_line)

    test_triples, words_indexes, indexes_words = load_triples_from_txt(os.path.join(folder, 'test.txt'),
    ...
    entity2id, relation2id, id2entity, id2relation = build_id_map(folder)
