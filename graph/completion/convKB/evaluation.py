...
def evaluate(test, embedding, embedding_dim, entity_array, metrics, options):
    # Parameters
    # ==================================================
    run_folder = "./"
    if embedding_dim == 50:
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if testIdx < (num_splits - 1):
                ...
                head_results = test_prediction(x_test[batch_test * testIdx: len_test],
                                               y_test[batch_test * testIdx: len_test],
                                               head_or_tail='head')
                tail_results = test_prediction(x_test[batch_test * testIdx: len_test],
                                               y_test[batch_test * testIdx: len_test],
                                               head_or_tail='tail')

            results = [(a+b)/((2 * len_test)) for a,b in zip(head_results, tail_results)]
            
            print(_file, 'mr, mrr, hits@10 --> ', results)#np.sum(results, axis=0) / (2 * len_test))
