...
flags.DEFINE_string("save_path", '/Users/anshuman786/PycharmProjects/CSCI548-Project/ckpt/RNNKM', "Directory to write the model and "
                                               "training summaries.")

flags.DEFINE_string("data_path", '/Users/anshuman786/PycharmProjects/CSCI548-Project/', "Training text file. "
...
class Trainer(object):
    ...
    def __init__(self, options: Options, session, init_tensor=True):
        ...
        #data_path='/Users/anshuman786/Desktop/Grad/Project548/DSKG-master/data/FB15k/'
        ...
        if os.path.exists(handled_path):
            ...
            #print("number of entities ", self._entity_num)
        else:
        '''
        self.read_data()
        self.merge_id()
    ...
    def train(self, t_labels=None, r_labels=None):
        ...
        for i in range(num_batch):
            ...
            self.saver.save(self._session,'./saved_variable')


            #for i in range(0,len(vals['label'])):
            #    print(vals['label'][i]," ",feed_dict[self._label][i])
    ...
    def predict(self):
        opts = self._options
        data = self._test_data[['h_id', 'r_id', 't_id']].values
        data = self.sample(data)
        ...
        for i in range(num_batch):
            one_batch_data = data[i * opts.batch_size: (i + 1) * opts.batch_size]
            if i > 0: tf.get_variable_scope().reuse_variables()
            print(type(one_batch_data[:,2]))
            #y = tf.placeholder(tf.int32,[self._options.batch_size])
            #y=tf.nn.softmax(tf.variable())
            #data_tf = tf.convert_to_tensor(one_batch_data[:, 2])

            feed_dict = {}
            feed_dict[self._eval_e] = one_batch_data[:, 0]
            feed_dict[self._eval_r] = one_batch_data[:, 1]
            #feed_dict[self._label] = one_batch_data[:, 2]

            #vals = self._session.run(fetches, feed_dict)
            self.saver.restore(self._session,'./saved_variable')
            prediction=tf.argmax(self._entity_probs,1)
            #vals=self._session.run(fetches, feed_dict)
            #print(vals)
            #print("prediction= ",prediction.eval(feed_dict,session=self._session))
            #print(one_batch_data[:,2])

            if(counter==0):
                counter=1
                predictions= prediction.eval(feed_dict,session=self._session)
            else:
               predictions= predictions+prediction.eval(feed_dict,session=self._session)
...
class RespectiveTrainer(Trainer):
    def build_graph(self, use_bn=True):
        ...
        #print("relation output= ", relation_output)
