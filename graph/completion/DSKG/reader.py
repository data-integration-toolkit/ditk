...
class FreeBaseReader(Reader):
    ...
    def read_data(self):
        ...
        #path='/Users/anshuman786/PycharmProjects/CSCI548-Project/'
        tr = pd.read_csv(path + 'train', header=None, sep='\t', names=['h', 'r', 't'])
        te = pd.read_csv(path + 'test', header=None, sep='\t', names=['h', 'r', 't'])
        val = pd.read_csv(path + 'valid', header=None, sep='\t', names=['h', 'r', 't'])

        e_id = pd.read_csv(path + 'entity2id', header=None, sep='\t', names=['e', 'eid'])
        e_id = pd.Series(e_id.eid.values, index=e_id.e.values)
        r_id = pd.read_csv(path + 'relation2id', header=None, sep='\t', names=['r', 'rid'])
