...
def parse_args(input = "" ):
    ...
    if input == "":
        parser.add_argument('--input', nargs='?', default='data2.txt',
    ...
    parser.add_argument('--type_size', type=int, default=edge,
...
def edge2vec_walk(G, walk_length, start_link,matrix):
    ...
    while len(walk) < walk_length:# here we may need to consider some dead end issues
        ...
        end_direction = 0
        # print "*******************************************"
        # print G.degree(start_node)
        # print G.degree(end_node)
        ...
        if start_direction == 0 and end_direction == 0:
            prob = 0
        else:
...
def update_trans_matrix(walks,type_size,evaluation_metric):
    ...
    for walk in walks:
        ...
        for i in range(type_size):
            ...
            # print "cu2rr_repo[i]: ",curr_repo[i],type(curr_repo[i])
            if i in curr_repo:
