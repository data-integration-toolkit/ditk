...
INPUT_FILE_DIR = "D:\\USC\\CS548\\ditk\\graph\\embedding\\complex\\test\\"
...
def main(input_file_path):
    ...
    # retrieve embeddings
    sub_re_em, sub_im_em = algorithm.retrieve_entity_embeddings(input_subjects)
    rel_re_em, rel_im_em = algorithm.retrieve_relations_embeddings(input_relations)
    obj_re_em, obj_im_em = algorithm.retrieve_entity_embeddings(input_objects)

    # output the vectors to a file
    with open(file_names['output'], "w") as f:
        for i in range(len(input_subjects)):
            f.write("{0}: {1}\n".format(input_subjects[i], sub_re_em[i]))
            f.write("{0}: {1}\n".format(input_relations[i], rel_re_em[i]))
            f.write("{0}: {1}\n".format(input_objects[i], obj_re_em[i]))
            f.write("\n\n")
