...
class mice(Mids, Imputation):
    ...
    sample_posterior : boolean, default=False
        ...
        ``True`` if using ``test`` for multiple imputations.
    ...
    def impute(self, input, trained_model=None):
        """
    ...
    def fit_transform(self, X, y=None):
        ...
        if self.verbose > 0:
            print("[test] Completing matrix with shape %s"
                  % (X.shape,))
        start_t = time()
        for i_rnd in range(self.n_iter):
            ...
            if self.verbose > 0:
                print('[test] Ending imputation round '
    ...
    def transform(self, X):
        ...
        i_rnd = 0
        if self.verbose > 0:
            print("[mice] Completing matrix with shape %s"
                  % (X.shape,))
        start_t = time()
