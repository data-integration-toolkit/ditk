...
if __name__ == '__main__':
    tf.app.flags.DEFINE_string('vocab_dir', 'data/cdr/processed/just_train_2500/protos', 'tsv file containing string data')
    tf.app.flags.DEFINE_string('kb_train', '',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('positive_train', 'data/cdr/processed/just_train_2500/protos/positive_*train*.txt.proto',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('negative_train', 'data/cdr/processed/just_train_2500/protos/negative_*train*_filtered.txt.proto',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('positive_dist_train', '',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('negative_dist_train', '',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('positive_test', 'data/cdr/processed/just_train_2500/protos/positive_*train*.txt.proto',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('negative_test', 'data/cdr/processed/just_train_2500/protos/negative_*train*_filtered.txt.proto',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('positive_test_test', 'data/cdr/processed/just_train_2500/protos/positive_*test.txt.proto',
                               'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('negative_test_test', 'data/cdr/processed/just_train_2500/protos/negative_*test_filtered.txt.proto',
                                'file pattern of proto buffers generated from ../src/processing/tsv_to_tfrecords.py')
    tf.app.flags.DEFINE_string('ner_train', 'data/cdr/processed/just_train_2500/protos/ner_CDR_train.txt.proto',
                               'file pattern of proto buffers generated from ../src/processing/ner_to_tfrecords.py')
    tf.app.flags.DEFINE_string('ner_test', 'data/cdr/processed/just_train_2500/protos/ner_CDR_dev.txt.proto',
                               'file pattern of proto buffers generated from ../src/processing/ner_to_tfrecords.py')
    tf.app.flags.DEFINE_string('embeddings', 'data/embeddings/just_train_2500_64d', 'pretrained word embeddings')
    ...
    tf.app.flags.DEFINE_string('logdir', 'LOG_DIR', 'save logs and models to this dir')
    ...
    tf.app.flags.DEFINE_string('model_type', 'classifier', 'optimizer to use')
    tf.app.flags.DEFINE_string('text_encoder', 'transformer_cnn_all_pairs', 'optimizer to use')
    ...
    tf.app.flags.DEFINE_string('mode', 'evaluate', 'train, evaluate, analyze')
    tf.app.flags.DEFINE_string('master', '', 'use for Supervisor')
    tf.app.flags.DEFINE_string('doc_filter', 'data/cdr/CDR_pubmed_ids/CDR_Train_Dev_pubmed_ids.txt', 'file to dev doc ids to split between train and test')
    ...
    tf.app.flags.DEFINE_string('export_file', 'data/cdr/output.txt', 'export predictions to this file in biocreative VI format')

    tf.app.flags.DEFINE_boolean('norm_entities', False, 'normalize entitiy vectors to have unit norm')
    tf.app.flags.DEFINE_boolean('bidirectional', True, 'bidirectional lstm')
    ...
    tf.app.flags.DEFINE_boolean('in_memory', True, 'load data in memory')
    ...
    tf.app.flags.DEFINE_float('lr', .0005, 'learning rate')
    tf.app.flags.DEFINE_float('epsilon', 1e-4, 'epsilon for adam optimizer')
    tf.app.flags.DEFINE_float('beta2', 0.9, 'beta2 adam optimizer')
    ...
    tf.app.flags.DEFINE_float('clip_norm', 10, 'clip gradients to have norm <= this')
    tf.app.flags.DEFINE_float('text_weight', 1.0, 'weight for text updates')
    tf.app.flags.DEFINE_float('ner_weight', 10.0, 'weight for text updates')
    tf.app.flags.DEFINE_float('ner_prob', 0.5, 'probability of drawing a text batch vs kb batch')
    tf.app.flags.DEFINE_float('text_prob', 1.0, 'probability of drawing a text batch vs kb batch')
    ...
    tf.app.flags.DEFINE_float('noise_std', 0.1, 'add noise to gradients from 0 mean gaussian with this std')
    tf.app.flags.DEFINE_float('train_dev_percent', .85, 'use this portion of dev as additional train data')
    ...
    tf.app.flags.DEFINE_float('pos_noise', .33, 'increase variance weight by this value each step')
    tf.app.flags.DEFINE_float('neg_noise', .20, 'increase variance weight by this value each step')

    tf.app.flags.DEFINE_float('word_dropout', .5, 'dropout keep probability for word embeddings')
    tf.app.flags.DEFINE_float('word_unk_dropout', 0.85, 'dropout keep probability for word embeddings')
    tf.app.flags.DEFINE_float('pos_unk_dropout', 1.0, 'dropout keep probability for position embeddings')
    tf.app.flags.DEFINE_float('lstm_dropout', .95, 'dropout keep probability for lstm output before projection')
    tf.app.flags.DEFINE_float('final_dropout', .35, 'dropout keep probability for final row and column representations')
    ...
    tf.app.flags.DEFINE_integer('kb_vocab_size', 2, 'learning rate')
    tf.app.flags.DEFINE_integer('text_batch', 32, 'batch size')
    tf.app.flags.DEFINE_integer('eval_batch', 32, 'batch size')
    tf.app.flags.DEFINE_integer('kb_batch', 16, 'batch size')
    tf.app.flags.DEFINE_integer('ner_batch', 16, 'batch size')
    tf.app.flags.DEFINE_integer('token_dim', 64, 'token dimension')
    tf.app.flags.DEFINE_integer('lstm_dim', 64, 'lstm internal dimension')
    tf.app.flags.DEFINE_integer('embed_dim', 64, 'row/col embedding dimension')
    tf.app.flags.DEFINE_integer('position_dim', 0, 'position relative to entities in lstm embedding')
    tf.app.flags.DEFINE_integer('text_epochs', 10000, 'train for this many text epochs')
    tf.app.flags.DEFINE_integer('kb_epochs', 100000, 'train for this many kb epochs')
    tf.app.flags.DEFINE_integer('kb_pretrain', 0, 'pretrain kb examples for this many steps')
    tf.app.flags.DEFINE_integer('block_repeats', 2, 'apply iterated blocks this many times')
    ...
    tf.app.flags.DEFINE_integer('eval_every', 3200, 'eval every k steps')
    tf.app.flags.DEFINE_integer('max_steps', -1, 'stop training after this many total steps')
    tf.app.flags.DEFINE_integer('max_seq', 2000, 'maximum sequence length')
    tf.app.flags.DEFINE_integer('max_decrease_epochs', 33, 'stop training early if eval doesnt go up')
    tf.app.flags.DEFINE_integer('num_classes', 2, 'number of classes for multiclass classifier')
