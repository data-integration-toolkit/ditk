...
from entity_linkage.typing.oneShotRelationalLearning.data_loader import *
from entity_linkage.typing.oneShotRelationalLearning.matcher import *
...
class Trainer(object):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id, self.e1rel_e2):
            ...
            if self.batch_nums % self.eval_every == 0:
                ...
                if self.batch_nums % (4 * self.eval_every) == 0:
                     ...
                     self.writer.add_scalar('MRR-test', mrr_, self.batch_nums)

            if self.batch_nums > 10 and (self.batch_nums % self.log_every) == 0:
                #self.save()
                logging.info('AVG. BATCH_LOSS: {} AT STEP {}'.format(np.mean(losses), self.batch_nums))
    ...
    def get_values(self) :
        ...
        for query_ in test_tasks.keys():
            ...
            all_support_pairs.append(support_pairs)
            ...
            for triple in test_tasks[query_][few:]:
                ...
                all_query_pairs.append(query_pairs)
        return all_query_pairs, all_support_pairs
    ...
    def get_score(self, query_pairs, support_pairs) :
        self.load()
        support = Variable(torch.LongTensor(support_pairs)).cuda()
        query = Variable(torch.LongTensor(query_pairs)).cuda()
        ...
        return hits10, hits5, hits1, mrr

    
    def test_(self):
        self.load()
        logging.info('Pre-trained model loaded')
        #self.eval(mode='dev', meta=self.meta, return_data=return_data)
        return self.eval(mode='test', meta=self.meta)


def get_scores(args, all_query_pairs, all_support_pairs):
    hits10 = []
    hits5 = []
    hits1 = []
    mrr = []
    trainer = Trainer(args)
    for query_pairs, support_pairs in zip(all_query_pairs, all_support_pairs) :
        hits10_, hits5_, hits1_, mrr_ = trainer.get_score(query_pairs, support_pairs)
        hits10.append(hits10_)
        hits5.append(hits5_)
        hits1.append(hits1_)
        mrr.append(mrr_)
...
def main(args) :
    ...
    trainer = Trainer(args)
    if args.test and args.get_data:
        return trainer.get_values()
    elif args.test :
        return trainer.test_()
    else:
        trainer.train()
    # trainer.eval()
