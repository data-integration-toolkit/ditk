...
def candidate_triples(dataset):
    ...
    all_reason_relations = []
    all_reason_relations.append(train_tasks.keys())
    all_reason_relations.append(dev_tasks.keys())
    all_reason_relations.append(test_tasks.keys())

    all_reason_relation_triples = []
    all_reason_relation_triples.append(train_tasks.values())
    all_reason_relation_triples.append(dev_tasks.values())
    all_reason_relation_triples.append(test_tasks.values())
    ...
    for rel, triples in zip(all_reason_relations, all_reason_relation_triples):
        ...
        for type_ in possible_types:
            candidates.append(list(type2ents[type_]))
...
def for_filtering(dataset, save=False):
    ...
    few_triples = []
    counter = 0
    for _ in train_tasks.values():
        for triple in _ :
            few_triples.append(triple)
    for _ in dev_tasks.values():
        for triple in _ :
            few_triples.append(triple)
    for _ in test_tasks.values():
        for triple in _ :
            few_triples.append(triple)
