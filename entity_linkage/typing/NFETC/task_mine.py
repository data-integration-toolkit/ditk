...
class Task:
    ...
    # def __init__(self, model_name, data_name, cv_runs, params_dict, logger, data_preprocessing=False):

        # if data_preprocessing:
        #     self.add_data_preprocessing(model_name, data_name, cv_runs, params_dict, logger)
        ...
        print("In task.py: self.train_set, self.full_test_set")
        print(len(self.train_set), ",", len(self.train_set[0]))
        print(len(self.full_test_set), ",", len(self.full_test_set[0]))


    # def add_data_preprocessing(self, model_name, data_name, cv_runs, params_dict, logger):
    #     print("Loading data...")
    #     if data_name == "wiki":
    #         words_train, mentions_train, positions_train, labels_train = data_utils.load(config.WIKI_TRAIN_CLEAN)
    #         words, mentions, positions, labels = data_utils.load(config.WIKI_TEST_CLEAN)
    ...
    def add_get_scores(self, test_data, preds, save=True):
        ...
        print("len : ", len(labels_test))
        print("labels_test : ", labels_test)
        print("preds : ", preds)

        if save:
            os.makedirs(config.OUTPUT_DIR, exist_ok=True) # ADDED -> Check?
    ...
    def get_scores(self, preds, save=True):
        preds = [label_path(self.id2type[x]) for x in preds]
        def vec2type(v):
            s = []
            for i in range(len(v)):
    ...
    def add_evaluate(self, test_data): # full=True
        ...
        for i in range(self.cv_runs):
            ...
            print("type of full_test_set: ", type(self.full_test_set), "len of full_test_set", len(self.full_test_set))
            print("type of test_data: ", type(test_data), "len of test_data", len(test_data))

            # 여기 밑 프리딕에서 에러...
            preds = self.model.predict(sess, self.full_test_set) # test_data == self.full_test_set
            # preds = self.model.predict(sess, test_data)
            sess.close()
