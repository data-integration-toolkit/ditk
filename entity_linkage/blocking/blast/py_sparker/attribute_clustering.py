...
import math
from entity_linkage.blocking.blast.py_sparker.objects import Profile, KeyValue, KeysCluster
...
class AttributeClustering(object):
    ...
    def getHashes(strHash, numHashes, seed=1234):
        ...
        def getPart():
            ...
            return int(((a * strHash + b) % 2147495899) % sys.maxsize)
    ...
    def clusterSimilarAttributes(profiles, numHashes, targetThreshold, maxFactor=1.0, numBands=-1, keysToExclude=[],
        ...
        def makeSignature(data):
            attribute = data[0]
            tokens = data[1]
            signature = [sys.maxsize for x in range(0, numHashes)]
            for t in tokens:
                h = hashes.value[t]
