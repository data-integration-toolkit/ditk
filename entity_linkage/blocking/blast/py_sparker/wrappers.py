...
from entity_linkage.blocking.blast.py_sparker.objects import Profile, MatchingEntities, KeyValue
import json
...
class CSVWrapper(object):
    ...
    def loadProfiles(filePath, startIDFrom=0, separator=",", header=True, realIDField="", sourceId=0):
        ...
        def rowToAttributes(row):
            return list(filter(lambda k: not k.isEmpty(), [KeyValue(col, row[col]) for col in columnNames]))
        ...
        def getProfile(data):
            ...
            if len(realIDField) > 0:
                realID = "".join(list(map(lambda a: a.value, list(filter(lambda a: a.key == realIDField, attributes))))).strip()
